...
namespace server.Main.Service
{
  public class AzureStorage : IAzureStorage
  {
    ...
    // UPDATE BLOB DATA //
    public async Task<BlobResponseDto> UpdateAsync(IFormFile blob)
    {
      // Create new upload response object that we can return to the requesting method
      BlobResponseDto response = new();
      ...
      try
      {
        BlobClient file = container.GetBlobClient(blob.FileName);

        // Check if the file exists in the container
        if (!await file.ExistsAsync())
        {
          response.Status = $"File {blob.FileName} Not Found";
          response.Error = true;
          return response;
        }
        ...
        // Everything is OK and file got uploaded
        response.Status = $"File {blob.FileName} Updated Successfully";
        response.Error = false;
        response.Blob.Uri = client.Uri.AbsoluteUri;
        response.Blob.Name = client.Name;
      }
